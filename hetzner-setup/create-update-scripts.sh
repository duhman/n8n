...
# This script creates the update scripts directly on the server

cat > /opt/setup/hetzner-setup/check-n8n-version.sh << 'SCRIPT_EOF'
#!/bin/bash
...
echo "Current latest: 1.100.1"
echo "Current next: 1.101.0"
echo ""
echo "To update, run: ./update-n8n.sh [version]"
echo "Example: ./update-n8n.sh 1.100.1"
SCRIPT_EOF

chmod +x /opt/setup/hetzner-setup/check-n8n-version.sh
echo "Created check-n8n-version.sh"